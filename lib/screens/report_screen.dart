  ...
  Future<void> fetchReportData() async {
    try {
      ...
      final response = await Supabase.instance.client
          ...
          .select()
          .eq('user_id', user.id)
          .gte('symptom_date', from.toIso8601String())
          .lt('symptom_date', toExclusive.toIso8601String())
      ...
      debugPrint('REPORT RECORDS: ${data.length}');
  ...
  Future<void> generateAiSummary() async {
    ...
    if (apiKey == null || apiKey.isEmpty) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Gemini API Key غير موجود')));
      return;
    }
    ...
    try {
      ...
      final prompt =
          '''
أنت مساعد يقوم بإنشاء ملخص صحي مختصر لسجلات الأعراض التي أدخلها المستخدم.

فترة التقرير:
من ${_formatArabicDate(reportFromDate!)}
إلى ${_formatArabicDate(reportToDate!)}

حلل فقط السجلات الموجودة في هذه الفترة.

اكتب ملخصاً قصيراً جداً ومفيداً للطبيب يتضمن:
• أهم الأعراض المسجلة وأكثرها تكراراً.
• متوسط شدة الأعراض بشكل مختصر.
• الحالات المتكررة المهمة إن وجدت.
• الأدوية المستخدمة إن وجدت.
• أهم ملاحظة أو نمط واضح فقط إن وجد.

قواعد مهمة:
- لا يتجاوز الملخص 150 كلمة.
- استخدم 4 إلى 6 نقاط فقط.
- اجعل كل نقطة قصيرة وواضحة.
- لا تكرر المعلومات.
- لا تقدم تشخيصاً طبياً.
- لا تقترح مرضاً معيناً.
- لا تقدم علاجاً أو توصيات طبية.
- لا تخترع أي معلومات.
- اعتمد فقط على السجلات المقدمة.
- لا تذكر الأقسام التي لا تحتوي على معلومات.
...
''';
      ...
      if (response.statusCode != 200) {
        ...
        throw Exception('تعذر إنشاء الملخص الذكي، الرجاء المحاولة مرة أخرى');
      }
      ...
      final summary = candidates[0]['content']['parts'][0]['text'].toString();
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('حدث خطأ أثناء إنشاء الملخص: $error')),
      );
    }
  }
  // =========================================================
  ...
  void _buildReport(List<Map<String, dynamic>> data) {
    ...
    for (final record in data) {
      final name = (record['condition_name'] ?? 'غير محدد').toString();
    ...
    symptoms = sortedEntries.map((entry) {
      return <String, dynamic>{
        'title': entry.key,
        'count': entry.value,
        'progress': maxCount == 0 ? 0.0 : entry.value / maxCount,
    ...
    for (final record in data) {
      severityTotal += double.tryParse(record['severity'].toString()) ?? 0;
    }
    ...
    for (final record in data) {
      final medicine = record['medicine_name']?.toString().trim();
    ...
    final dates = data
        .map((record) => DateTime.tryParse(record['symptom_date'].toString()))
        .whereType<DateTime>()
        .toList();
                  ...
                  children: [
                    ...
                    if (isLoading)
                          ...
                          children: List.generate(symptoms.length, (index) {
                            ...
                            return Padding(
                              ...
                              child: _symptomProgress(
                                ...
                                title: symptom['title'],
                                count: symptom['count'],
                                progress: (symptom['progress'] as num)
                                    .toDouble(),
                                color: symptom['color'],
                                icon: symptom['icon'],
                              ),
                      ...
                      SizedBox(
                        ...
                        height: height * 0.065,
                        child: ElevatedButton.icon(
                          onPressed: isGeneratingAi ? null : generateAiSummary,
                          style: ElevatedButton.styleFrom(
                            elevation: 0,
                            backgroundColor: homePrimaryColor,
                            foregroundColor: whiteColor,
                          ...
                          icon: isGeneratingAi
                              ...
                              : Icon(Icons.auto_awesome, size: width * 0.05),
                          label: Text(
                            isGeneratingAi
                                ? 'جاري تحليل البيانات...'
                                : 'إنشاء الملخص الذكي',
      ...
      child: Container(
        ...
        child: Row(
          children: [
            ...
            Expanded(
              child: Text(
                date == null ? title : _formatArabicDate(date),
  ...
  Widget _statCard({
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final double cardHeight = (cardWidth * 0.72).clamp(95.0, 130.0);

        final double numberSize = (cardWidth * 0.27).clamp(25.0, 38.0);

        final double titleSize = (cardWidth * 0.14).clamp(13.0, 18.0);
  ...
  List<String> _splitAiSummaryForPdf(String text) {
    final cleaned = text.trim();

    if (cleaned.isEmpty) {
      return ['لم يتم إنشاء الملخص الذكي لهذا التقرير.'];
    }
    ...
    for (final paragraph in paragraphs) {
      ...
      for (int start = 0; start < value.length; start += maxCharacters) {
        final end = (start + maxCharacters < value.length)
            ? start + maxCharacters
            : value.length;
  ...
  Future<void> generatePdf() async {
    if (records.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('لا توجد بيانات لإنشاء التقرير')),
      );
      return;
    }
    ...
    try {
        ...
        pw.MultiPage(
          pageTheme: pw.PageTheme(
            ...
            buildBackground: (context) {
              return pw.FullPage(
                ignoreMargins: true,
                child: pw.Container(color: lighterPurple),
              );
            },
          ),
            ...
            return [
              ...
              pw.Container(
                ...
                decoration: pw.BoxDecoration(
                  ...
                  border: pw.Border.all(color: borderPurple, width: 0.7),
                ),
                child: pw.Center(
                  child: pw.Text(
                    'التقرير الصحي',
              ...
              // ==========================================
              pw.Table(
                border: pw.TableBorder.all(color: borderPurple, width: 0.7),
                children: [
                  pw.TableRow(
                    decoration: pw.BoxDecoration(color: lightPurple),
                    children: [
                      _pdfCell('أنواع الأعراض', boldFont),
                      _pdfCell('متوسط الشدة', boldFont),
                      _pdfCell('الحالات المتكررة', boldFont),
                    ],
                  ),

                  pw.TableRow(
                    children: [
                      _pdfCell('$symptomTypesCount', regularFont),
                      _pdfCell(averageSeverity.toStringAsFixed(1), regularFont),
                      _pdfCell('$repeatedCount', regularFont),
                    ],
                  ),
                ],
              ),
              ...
              ...symptoms.map((symptom) {
                return pw.Container(
                  ...
                  decoration: pw.BoxDecoration(
                    ...
                    border: pw.Border.all(color: borderPurple, width: 0.6),
                  ),
                  child: pw.Row(
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('حدث خطأ أثناء إنشاء PDF: $error')),
      );
    }
  }
    ...
    return Row(
      children: [
        Container(
          ...
          child: Icon(icon, color: homeDarkTextColor, size: width * 0.045),
        ),
  ...
  Widget _emptyCard(double width, double height) {
    return Container(
      padding: EdgeInsets.symmetric(
  ...
  Widget _errorCard(double width, double height) {
    return Container(
